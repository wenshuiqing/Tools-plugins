...
#include "UnityCG.cginc"  
	...
	v2f_withBlurCoords vertBlurVertical(appdata_img v)
	{
		...
		o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
		o.uv2[0] = v.texcoord + _MainTex_TexelSize.xy * float2(0.0, -1.5);
		o.uv2[1] = v.texcoord + _MainTex_TexelSize.xy * float2(0.0, -0.5);
		o.uv2[2] = v.texcoord + _MainTex_TexelSize.xy * float2(0.0, 0.5);
		o.uv2[3] = v.texcoord + _MainTex_TexelSize.xy * float2(0.0, 1.5);
		return o;
	}
	...
	v2f_withBlurCoords vertBlurHorizontal(appdata_img v)
	{
		v2f_withBlurCoords o;
		o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
		o.uv2[0] = v.texcoord + _MainTex_TexelSize.xy * float2(-1.5, 0.0);
		o.uv2[1] = v.texcoord + _MainTex_TexelSize.xy * float2(-0.5, 0.0);
		o.uv2[2] = v.texcoord + _MainTex_TexelSize.xy * float2(0.5, 0.0);
		o.uv2[3] = v.texcoord + _MainTex_TexelSize.xy * float2(1.5, 0.0);
		return o;
	}
	...
	return color;
		...
		float4 fragMaxWithPain(v2f_withMaxCoords i) : COLOR
	{
		float4 color = tex2D(_MainTex, i.uv.xy);
	...
	return saturate(color + float4(0.25, 0, 0, 0) - THRESHHOLD) * INTENSITY;
	}
	...
	color = max(color, tex2D(_MainTex, i.uv2[2]));
	color = max(color, tex2D(_MainTex, i.uv2[3]));
	return saturate(color + float4(0, 0, 0, 0) - THRESHHOLD) * INTENSITY;
	}
