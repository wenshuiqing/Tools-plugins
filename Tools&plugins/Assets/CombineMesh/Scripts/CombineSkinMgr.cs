...
public class MyCombineSkinMgr
{
    ...
    public void CombineObject(SkinnedMeshRenderer skeleton, SkinnedMeshRenderer[] meshes, Material newMaterial)
    {
        //  Profiler.BeginSample("111111111111111111");
        // Fetch all bones of the skeleton
        List<Transform> transforms = new List<Transform>();
        List<CombineInstance> combineInstances = new List<CombineInstance>();//the list of meshes
        List<Transform> bones = new List<Transform>();//the list of bones

        transforms.AddRange(skeleton.gameObject.GetComponentsInChildren<Transform>(true));
        ...
        for (int i = 0; i < meshes.Length; i++)
        {
            ...
            Debug.LogError(smr.bones.Length);
        ...
        //   Profiler.EndSample();
        skeleton.sharedMesh = new Mesh();

        skeleton.sharedMesh.CombineMeshes(combineInstances.ToArray(), true, false);// Combine meshes
        skeleton.bones = bones.ToArray();// Use new bones

        skeleton.material = newMaterial;
